...
class Enemy2(object):
    ...
    def loadModel(self):
        self.actor = Actor("models/armored",{'tread':'models/armored'})
        self.actor.setPlayRate(2.0,'tread')
    ...
    def fire(self,game):
        ...
        if math.fabs(h) < 15 and self.timer <= 0:
            ## Put firing code here
            b1 = mortar.Mortar(self,game,manoffset = (0,0,20))
            b1.mortarNP.setH(b1.mortarNP.getH() + 90)
            self.timer = self.fire_rate
        else:
            self.timer -= 1
        self.actor.setH(h1)
